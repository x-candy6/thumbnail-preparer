...
def get_watermark():
    watermark = helper.select_files()
    return watermark

def compress_files(files, compress_mag=100):
...
def add_border(files, color):

   Logger.info("running add_border()")
   #color = input("Enter the color name(common colors")
   for image in files:
      ...
      cmd = f"magick convert {image} -bordercolor {color} -border {bw}x{bl} {image}"
      print(cmd)
      subprocess.run(cmd, shell=True)

def add_watermark(files, watermark_path):
   Logger.info("running add_watermark()")
   #watermark = helper.select_files()[0]
   for image in files:
      cmd = f"magick composite -gravity center {watermark_path} {image} {image}"
      print(cmd)
      subprocess.run(cmd, shell=True)
...
def generate_filelist():
    ...
    for folder in folders:
        with open(f"{folder}/urls.txt", 'a', encoding="utf-8") as output:
            ...
            for filename in os.listdir(folder):
                # Read the contents of the file and append it to the output file
                output.write(f"https://{cname}/{filename}")
                output.write('\n') 
